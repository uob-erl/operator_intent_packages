...
def run():
    ...
    while not rospy.is_shutdown():
        ...
        targets_p91011 = [g_prime, g9, g10, g11]
        t = [g_prime[0], g_prime[1], g0[0], g0[1], g1[0], g1[1], g2[0], g2[1]]
        t_p91011 = [g9[0], g9[1], g10[0], g10[1], g11[0], g11[1]]
        # goals = np.array([])
        # for i in t:
        #     q = 1*t
        #     goal = np.append(goals, q)
        # goals = goal  # array of goals (MAP FRAME)
        ...
        if check < 18:
            ...
            for v in new_goals:
                z = 1*new_goals
                new = np.append(new, z)
            news = new  # array of new goals (ROBOT FRAME) .. prime,0,1,2
            ...
            ind_pos_x = [0, 2, 4, 6]
            ind_pos_y = [1, 3, 5, 7]
            dx = news - robot_base[0]
            dy = news - robot_base[1]
            Dx = dx[ind_pos_x]
            Dy = dx[ind_pos_y]
            ...
            new_p91011 = np.array([])
            new_goals_p91011 = [g_prime_new[0], g_prime_new[1], g9_new[0], g9_new[1], g10_new[0], g10_new[1], g11_new[0], g11_new[1]]
            for j in new_goals_p91011:
                w = 1*new_goals_p91011
                new_p91011 = np.append(new_p91011, w)
            news_p91011 = new_p91011 # array of new goals (ROBOT FRAME) .. prime,9,10,11

            measure_p91011 = np.array([])
            for x in targets_p91011:
                dist_p91011 = distance.euclidean(robot_coord, x)
                measure_p91011 = np.append(measure_p91011, dist_p91011)
            dist_p91011 = measure_p91011

            robot_base = [0, 0]
            ind_pos_x = [0, 2, 4, 6]
            ind_pos_y = [1, 3, 5, 7]
            dx = news_p91011 - robot_base[0]
            dy = news_p91011 - robot_base[1]
            Dx = dx[ind_pos_x]
            Dy = dx[ind_pos_y]
            angle_p91011 = np.arctan2(Dy, Dx) * 180 / np.pi
            Angle_p91011 = abs(angle_p91011)

            # BAYES' FILTER
            likelihood = compute_like(dist_p91011, Angle_p91011, m, k)
            summary = compute_conditional(cond, prior)
            posterior = compute_post(likelihood, summary)
            index = np.argmax(posterior)
            prior = posterior
            ...
            if index == 0:
                ...
                x_stoxos = g9[0] + offset
                y_stoxos = g9[1] - offset
            elif index == 2:
                x_stoxos = g10[0] + offset
                y_stoxos = g10[1] - offset
            else:
                x_stoxos = g11[0] + offset
                y_stoxos = g11[1] - offset
            ...
            rospy.loginfo("NAV_goal: %s", g_prime)
            rospy.loginfo("path_length %s", path_length)
            rospy.loginfo("Distance: %s", dist_p91011)
            rospy.loginfo("Angles: %s", Angle_p91011)
            rospy.loginfo("Posterior: %s", posterior)
            rospy.loginfo("Potential Goal is %s", index)
