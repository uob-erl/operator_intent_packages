...
'''
alternative script to the main one .. in case of emergency
HERE there is no class called .. all BOIR's functionalities are defined
as functions
'''
...
def calc_likelihood(Angle, path, wA, wP, maxA, maxP):  # computing likelihood
    a = Angle / maxA
    p = path / maxP
...
def calc_conditional(data_cpt, prior):
    c_c = np.matmul(data_cpt, np.transpose(prior))
    return c_c


def calc_posterior(likelihood, conditional): # max posterior and publisher shoud be IGNORING ghost
    output = likelihood * conditional
    c_p = output / np.sum(output)
    print('POSTERIOR', c_p)
    print("maximum posterior (ghost excluded): ", np.max(c_p[1:]))
    return c_p
...
if __name__=='__main__':
    try:
        ...
        x_robot = y_robot = x_nav = y_nav = path_length = post_avg = post_sum = 0
            ...
            if check_area_FINAL < threshold_click:  # here starts the experiment
                ...
                if circle_area_A:
                    ...
                    while circle_area_A:
                        ...
                        likelihood = calc_likelihood(angle, path, wA, wP, maxA, maxP)
                        conditional = calc_conditional(data_cpt, prior)
                        posterior = calc_posterior(likelihood, conditional)
                        #index = np.argmax(posterior)
                        #max_post_pub.publish(np.max(posterior))
                        ...
                        ## modified simple moving average .. append next and remove first and so on
                        # if cnt < number:
                        ...
                        ## modified simple moving average .. here stored_values initialized as stored_values = deque(maxlen=number+1)
                        # stored_values.append(np.max(posterior[1:]))
                    ...
                    while circle_area_B:
                        ...
                        ## modified simple moving average .. here stored_values initialized as stored_values = deque(maxlen=number+1)
                        # stored_values.append(np.max(posterior[1:]))
                        # if cnt % number == 0 and cnt != 0:  # modified SMA (Simple Moving Average)
                        #     print('iteration ', cnt)
                        #     print("stored_values ", stored_values)
                        #     max_post_avg = round(np.sum(stored_values) / len(stored_values), 4)
                        #     print("value to be published", max_post_avg)
                    ...
                    while circle_area_A == False and circle_area_B == False:
                        ...
                        #max_post_pub.publish(post_avg)
                        #
                        # EMA (Exponential Moving Average) attempts to correct for lag time by giving more weight to recent value moves than older ones
                        if cnt <= number:
                            post_sum += np.max(posterior[1:])
                            post_avg = round(post_sum / cnt, 4)
